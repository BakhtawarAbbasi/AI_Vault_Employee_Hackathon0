...
transactions and tasks to report revenue and bottlenecks, transforms the AI from a chatbot into 
...
rather than waiting for user input and "Ralph Wiggum" (a Stop hook pattern) to keep it working 
until done. 
...
Claude Code
Purpose 
Active subscription (Pro or Use Free Gemini API 
...
b. 
Local owns: approvals, WhatsApp session, payments/banking, and final “send/post” 
actions 
...
Cloud writes updates to /Updates/ (or /Signals/), and Local merges them into 
Dashboard.md. 
b. 
c. 
For Vault sync (Phase 1) use Git (recommended) or Syncthing. 
Claim-by-move rule: first agent to move an item from /Needs_Action to 
...
 The Nerve Center (Obsidian): Acts as the GUI (Graphical User Interface) and Long
Term Memory. 
o Dashboard.md: Real-time summary of bank balance, pending messages, and active 
business projects. 
o Company_Handbook.md: Contains your "Rules of Engagement" (e.g., "Always be 
...
its File System tools to read your tasks and write reports. The Ralph Wiggum loop (a Stop 
hook) keeps Claude iterating until multi-step tasks are complete. 
...
## Suggested Actions - [ ] Reply to sender - [ ] Forward to relevant party - [ ] Archive after processing 
''' 
...
APPROVAL_REQUIRED_Payment_Client_A.md. It will not click "Send" until you move 
that file to the /Approved folder. 
...
slack-mcp 
Scheduling 
Send messages, read channels Team communication 
...
status: pending --- 
## Payment Details - Amount: $500.00 - To: Client A (Bank: XXXX1234) - Reference: Invoice #1234 
## To Approve 
...
```bash 
# Start a Ralph loop 
/ralph-loop "Process all files in /Needs_Action, move to /Done when complete" \ --completion-promise "TASK_COMPLETE" \ --max-iterations 10 
``` 
...
Continuous 
cron (Mac/Linux) or Task Scheduler 
(Win) calls Claude. 
Lead Capture: Watch WhatsApp 
for keywords like "Pricing." 
Project
Based 
Python watchdog script monitoring 
the /Inbox folder. 
Q1 Tax Prep: Categorize 3 
months of business expenses. 
Manual drag-and-drop of a file into 
the /Active_Project folder. 
...
o Revenue: Total earned this week. 
o Bottlenecks: Tasks that took too long. 
o Proactive Suggestion: "I noticed we spent $200 on software we don't use; shall I 
...
### Revenue Target - Monthly goal: $10,000 - Current MTD: $4,500 
...
Flag for review if: - No login in 30 days - Cost increased > 20% - Duplicate functionality with another tool 
Weekly Audit Logic 
...
## Revenue - **This Week**: $2,450 - **MTD**: $4,500 (45% of $10,000 target) - **Trend**: On track 
 
## Completed Tasks - [x] Client A invoice sent and paid - [x] Project Alpha milestone 2 delivered - [x] Weekly social media posts scheduled 
...
### Cost Optimization - **Notion**: No team activity in 45 days. Cost: $15/month. 
  - [ACTION] Cancel subscription? Move to /Pending_Approval 
 
### Upcoming Deadlines - Project Alpha final delivery: Jan 15 (9 days) - Quarterly tax prep: Jan 31 (25 days) 
 --- 
...
and Banking. 
o Playwright for "Computer Use" (interacting with websites for payments). 
...
Recovery Strategy 
Transient 
Network timeout, API rate limit Exponential backoff retry 
Authentication Expired token, revoked access Alert human, pause operations 
Logic 
Claude misinterprets message Human review queue 
Data 
...
https://agentfactory.panaversity.org/docs/AI-Tool-Landscape/claude-code-features-and
workflows  
...
Presentation 
Time 
https://docs.google.com/presentation/d/1UGvCUk1-O8m5i
aTWQNxzg8EXoKzPa8fgcwfNh8vRjQ/edit?usp=sharing  
Claude Code 
Fundamentals 
https://agentfactory.panaversity.org/docs/AI-Tool
Landscape/claude-code-features-and-workflows  
2 
hours 
3 
hour 
...
hour 
Agent Skills 
platform.claude.com/docs/en/agents-and-tools/agent
skills/overview 
Core Learning (During Hackathon) 
Topic 
Resource 
Claude + Obsidian Integration youtube.com/watch?v=sCIS05Qt79Y 
Building MCP Servers 
modelcontextprotocol.io/quickstart 
Claude Agent Teams 
youtube.com/watch?v=0J2_YGuNrDo 
Gmail API Setup 
developers.google.com/gmail/api/quickstart Docs 
...
## Steps - [x] Identify client: Client A (client_a@email.com) - [x] Calculate amount: $1,500 (from /Accounting/Rates.md) - [ ] Generate invoice PDF - [ ] Send via email (REQUIRES APPROVAL) - [ ] Log transaction 
...
## Recent Activity - [2026-01-07 10:45] Invoice sent to Client A ($1,500) 
...
daemon processes. They are designed to run indefinitely to poll for events1 . 
...
"Process Management" solves this by wrapping your scripts in a supervisor that ensures state 
persistence. 
...
the script on boot 3 . 
 Logging: It captures stdout/stderr to log files, which is critical for debugging silent failures 
over long periods. 
...
pm2 save 
pm2 startup 
Alternatively, the hackathon document suggests writing a custom Python "Watchdog" script that 
loops and checks PIDs, effectively building a primitive process manager yourself. 